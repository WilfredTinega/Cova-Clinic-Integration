...
const GROUPS = "cova_clinic_integration.cova_clinic_integration.doctype.clinic_test_schedule.clinic_test_schedule.test_group_filters";
    ...
    refresh(frm) {
        ...
        if (next || !(frm.doc.rounds || []).length) {
            frm.add_custom_button(__("Schedule Next Round"), () => {
                frm.call({ doc: frm.doc, method: "schedule_next_round", freeze: true,
                    freeze_message: __("Scheduling and sending to Cova…") })
                    .then((r) => {
                        frm.reload_doc();
                        const m = r.message || {};
                        let msg = __("Round {0}: {1} employees scheduled on {2}.", [m.round, m.scheduled, m.schedule]);
                        if ((m.on_leave || []).length) {
                            msg += "<br>" + __("{0} on leave will carry over to the next round: {1}",
                                [m.on_leave.length, m.on_leave.map((e) => frappe.utils.escape_html(e.employee_name)).join(", ")]);
                        }
