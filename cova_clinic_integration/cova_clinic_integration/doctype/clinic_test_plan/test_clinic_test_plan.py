...
class TestClinicTestPlan(IntegrationTestCase):
	def setUp(self):
		...
		settings.set(
			"test_groups",
			[{"group_name": "CV Sprayers", "test_package": "Annual Medical", "designation": ROLE}],
		)
	...
	def test_metrics_and_rounds(self):
		...
		self.assertEqual(
			[(str(r.scheduled_from), str(r.scheduled_to)) for r in plan.rounds],
	...
	def test_employees_on_leave_carry_over_to_the_next_round(self):
		...
		scheduled = frappe.get_all(
			"Clinic Test Request", filters={"clinic_test_schedule": first["schedule"]}, pluck="employee"
		)
		...
		self.assertEqual(
			frappe.get_all(
				"Clinic Test Request", filters={"clinic_test_schedule": second["schedule"]}, pluck="employee"
			),
			[self.away],
		)
