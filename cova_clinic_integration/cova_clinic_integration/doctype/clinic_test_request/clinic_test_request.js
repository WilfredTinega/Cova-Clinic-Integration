...
if (!frappe.cova.clinic_test_request_form) {
	...
	frappe.ui.form.on("Clinic Test Request", {
		refresh: function (frm) {
			...
			frm.add_custom_button(
				delivered ? __("Re-send to Clinic") : __("Send to Clinic"),
				function () {
			...
			frappe.confirm(
				__("Send this test request to Cova now?") +
					...
					frappe.utils.escape_html(
						frm.doc.employee || frm.doc.nationa_id || frm.doc.name
					),
				function () {
	...
	var cova_is_delivered = function (frm) {
	...
	var cova_precheck = function (frm) {
		const warnings = [];
		...
		if (frm.doc.member_type === "Active") {
			if (!frm.doc.employee) {
				frappe.msgprint({
					...
					message: __(
						"An Active request needs an Employee — Cova identifies the member by payroll number."
					),
					indicator: "red",
				});
				return null;
			...
			if (!frm.doc.payroll_number) {
				...
				warnings.push(
					__(
						"Payroll Number is blank — the Employee ID ({0}) will be sent as the member identifier.",
						[frm.doc.employee]
					)
				);
			}
			...
			frappe.msgprint({
				title: __("Cannot Send"),
				message: __(
					"A Pre Employment request needs a National ID — Cova identifies the candidate by it."
				),
				indicator: "red",
			});
			return null;
	...
	var cova_send_request = function (frm, delivered, confirmed) {
		...
		if (delivered) {
			warnings.unshift(
				__("Cova has already received this request. Sending again raises a second test.")
			);
		}
	...
	var cova_post_request = function (frm) {
		...
		fetch("/api/method/cova_clinic_api", {
			...
			.then(function (text) {
				...
				if (outcome.state === "failed") {
					frappe.msgprint({
						...
						message:
							__("Nothing was sent. Cova said: {0}", [
								frappe.utils.escape_html(outcome.reason),
							]) +
							"<br><br>" +
							__(
								"Fix the cause and use <b>Clinic → Send to Clinic</b>, or re-send it in bulk from the list view."
							),
						indicator: "red",
					});
				} else {
			...
			.catch(function (err) {
				frappe.dom.unfreeze();
				cova_log_attempt(frm.doctype, frm.doc.name, {
					state: "failed",
					reason: String(err),
				});
				frappe.msgprint({
	...
	var cova_read_outcome = function (data) {
	...
	var cova_log_attempt = function (doctype, name, outcome) {
		const content =
			outcome.state === "failed"
				? __("Cova send FAILED: {0}", [frappe.utils.escape_html(outcome.reason)])
				: outcome.state === "duplicate"
				? __("Cova reported this request as already raised: {0}", [
						frappe.utils.escape_html(outcome.reason),
				  ])
				: __("Sent to Cova.");
